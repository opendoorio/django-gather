...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        db.create_table('gather_event', (
            ...
            ('creator', self.gf('django.db.models.fields.related.ForeignKey')(related_name='events_created', to=orm['auth.User'])),
        ))
        db.send_create_signal('gather', ['Event'])

        # Adding M2M table for field attendees on 'Event'
        db.create_table('gather_event_attendees', (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
            ('event', models.ForeignKey(orm['gather.event'], null=False)),
            ('user', models.ForeignKey(orm['auth.user'], null=False))
        ))
        db.create_unique('gather_event_attendees', ['event_id', 'user_id'])

        # Adding M2M table for field organizers on 'Event'
        db.create_table('gather_event_organizers', (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
            ('event', models.ForeignKey(orm['gather.event'], null=False)),
            ('user', models.ForeignKey(orm['auth.user'], null=False))
        ))
        db.create_unique('gather_event_organizers', ['event_id', 'user_id'])


    def backwards(self, orm):
        # Deleting model 'Event'
        db.delete_table('gather_event')

        # Removing M2M table for field attendees on 'Event'
        db.delete_table('gather_event_attendees')

        # Removing M2M table for field organizers on 'Event'
        db.delete_table('gather_event_organizers')
